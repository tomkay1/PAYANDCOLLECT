...
public class CBatchTradeSrv {
	...
	public UnionpayBatchCollection buildUnionpayBatchCollection(List<UnionpayCollection> toBeSentOrder) {
		...
		unionpayBatchCollection.setBatchNo("0001");
	...
	private BatchCollectionRequest buildBatchCollectionRequest(List<UnionpayCollection> unionpayCollections) {
		...
		for (UnionpayCollection unionpayCollection : unionpayCollections) {
			...
			unionpayCollection.setOperID(unionpayCollection.getOperID());
			unionpayCollection.setCurrencyCode(unionpayCollection.getCurrencyCode());
			unionpayCollection.setTxnAmt(txnAmt);
			unionpayCollection.setAccType(unionpayCollection.getAccType());
			unionpayCollection.setAccNo(unionpayCollection.getAccNo());
			unionpayCollection.setCustomerNm(unionpayCollection.getCustomerNm());
			unionpayCollection.setBizType(unionpayCollection.getBizType());
			unionpayCollection.setCertifTp(unionpayCollection.getCertifTp());
			unionpayCollection.setCertifId(unionpayCollection.getCertifId());
			unionpayCollection.setPhoneNo(unionpayCollection.getPhoneNo());
			unionpayCollection.setPostscript(unionpayCollection.getPostscript());

			batchCollectionRequest.addContent(requestContent);
			totalAmt.add(new BigDecimal(txnAmt));
		}
