...
import com.mybank.pc.advance.model.base.BaseUnionpayAdvanceQuery;
import com.mybank.pc.advance.model.sender.AdvanceQueryRequestBuilder;
...
public class UnionpayAdvanceQuery extends BaseUnionpayAdvanceQuery<UnionpayAdvanceQuery> {
	public static final UnionpayAdvanceQuery dao = new UnionpayAdvanceQuery().dao();

	private SendProxy sendProxy;

	public UnionpayAdvanceQuery assemblyQueryRequest() {
		this.sendProxy = new AdvanceQueryRequestBuilder(this).build();
	...
	public List<UnionpayAdvanceQuery> findUnionpayAdvanceQuery() {
		return findUnionpayAdvanceQuery(this);
	}

	public static List<UnionpayAdvanceQuery> findByOrderId(String orderId) {
		SqlPara sqlPara = Db.getSqlPara("advance.findUnionpayAdvanceQuery", Kv.by("orderId", orderId));
		return UnionpayAdvanceQuery.dao.find(sqlPara);
	}

	public static List<UnionpayAdvanceQuery> findUnionpayAdvanceQuery(UnionpayAdvanceQuery param) {
		SqlPara sqlPara = Db.getSqlPara("advance.findUnionpayAdvanceQuery", param);
		return UnionpayAdvanceQuery.dao.find(sqlPara);
	}
