...
public class AdvanceRequestBuilder implements SenderBuilder {

	private UnionpayAdvance UnionpayAdvance;

	public AdvanceRequestBuilder(UnionpayAdvance UnionpayAdvance) {
		this.UnionpayAdvance = UnionpayAdvance;
	}

	@Override
	public SendProxy build() {
		SendProxy sendProxy = new SendProxy(UnionpayAdvance.getMerId());
		...
		// 版本号
		contentData.put("version", UnionpayAdvance.getVersion());
		// 字符集编码 可以使用UTF-8,GBK两种方式
		contentData.put("encoding", UnionpayAdvance.getEncoding());
		// 签名方法 目前只支持01-RSA方式证书加密
		contentData.put("signMethod", sendProxy.getSignMethod());
		// 交易类型
		contentData.put("txnType", UnionpayAdvance.getTxnType());
		// 交易子类型
		contentData.put("txnSubType", UnionpayAdvance.getTxnSubType());
		// 业务类型
		contentData.put("bizType", UnionpayAdvance.getBizType());
		// 渠道类型
		contentData.put("channelType", UnionpayAdvance.getChannelType());

		/*** 商户接入参数 ***/
		// 商户号码
		contentData.put("merId", UnionpayAdvance.getMerId());
		// 接入类型，商户接入固定填0，不需修改
		contentData.put("accessType", UnionpayAdvance.getAccessType());
		// 商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
		contentData.put("orderId", UnionpayAdvance.getOrderId());
		// 订单发送时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
		contentData.put("txnTime", UnionpayAdvance.getTxnTime());
		// 交易币种（境内商户一般是156人民币）
		contentData.put("currencyCode", UnionpayAdvance.getCurrencyCode());
		// 交易金额，单位分，不要带小数点
		contentData.put("txnAmt", UnionpayAdvance.getTxnAmt());
		// 账号类型
		contentData.put("accType", UnionpayAdvance.getAccType());
		...
		String accNoEnc = acpService.encryptData(UnionpayAdvance.getAccNo(), SDKConstants.UTF_8_ENCODING);
		...
		if (sdk == SDK.REALTIME_YS_2_SDK || sdk == SDK.REALTIME_YS_4_SDK) {
			...
			customerInfoMap.put("certifTp", UnionpayAdvance.getCertifTp()); // 证件类型
			customerInfoMap.put("certifId", UnionpayAdvance.getCertifId()); // 证件号码
			customerInfoMap.put("customerNm", UnionpayAdvance.getCustomerNm()); // 姓名
			String customerInfoStr = acpService.getCustomerInfoWithEncrypt(customerInfoMap, UnionpayAdvance.getAccNo(),
		...
		// 请求方保留域
		contentData.put("reqReserved", UnionpayAdvance.getReqReserved());
