...
import com.mybank.pc.admin.model.base.BaseTaxonomy;
...
public class Taxonomy extends BaseTaxonomy<Taxonomy> {
	...
	public List<Taxonomy> findAllListByModule(String module) {
		List<Taxonomy> list = dao.find("select * from s_taxonomy where parentId =0 and dAt is null and module=?",
				module);
		for (Taxonomy taxonomy : list) {
			findChildren(taxonomy);
		}
	...
	public List findByTitleAndPIdAndNEId(String title, Long pId, Long id) {
		return dao.find("select * from s_taxonomy where title=? and dAt is null and parentId=? and id<>?", title, pId,
				id);
	}
