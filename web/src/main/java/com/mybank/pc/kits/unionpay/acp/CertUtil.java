...
public class CertUtil {
	...
	static {
		try {
			...
			LogKit.error("init失败。（如果是用对称密钥签名的可无视此异常。）", e);
		}
	}

	public CertUtil(SDKConfig sdkConfig) {
	...
	private void initSignCert() {
		if (!"01".equals(sdkConfig.getSignMethod())) {
			LogKit.info("非rsa签名方式，不加载签名证书。");
			return;
		}
		if (sdkConfig.getSignCertPath() == null || sdkConfig.getSignCertPwd() == null
				|| sdkConfig.getSignCertType() == null) {
			LogKit.error("WARN: " + SDKConfig.SDK_SIGNCERT_PATH + "或" + SDKConfig.SDK_SIGNCERT_PWD + "或"
		...
		try {
			keyStore = getKeyInfo(sdkConfig.getSignCertPath(), sdkConfig.getSignCertPwd(), sdkConfig.getSignCertType());
			LogKit.info("InitSignCert Successful. CertId=[" + getSignCertId() + "]");
		} catch (IOException e) {
			LogKit.error("InitSignCert Error", e);
		}
	}
	...
	private void initMiddleCert() {
		LogKit.info("加载中级证书==>" + sdkConfig.getMiddleCertPath());
		if (!isEmpty(sdkConfig.getMiddleCertPath())) {
			middleCert = initCert(sdkConfig.getMiddleCertPath());
			LogKit.info("Load MiddleCert Successful");
		} else {
			LogKit.info("WARN: acpsdk.middle.path is empty");
		}
	}
	...
	private void initRootCert() {
		LogKit.info("加载根证书==>" + sdkConfig.getRootCertPath());
		if (!isEmpty(sdkConfig.getRootCertPath())) {
			rootCert = initCert(sdkConfig.getRootCertPath());
			LogKit.info("Load RootCert Successful");
		} else {
			LogKit.info("WARN: acpsdk.rootCert.path is empty");
		}
	}
	...
	private void initEncryptCert() {
		LogKit.info("加载敏感信息加密证书==>" + sdkConfig.getEncryptCertPath());
		if (!isEmpty(sdkConfig.getEncryptCertPath())) {
			encryptCert = initCert(sdkConfig.getEncryptCertPath());
			LogKit.info("Load EncryptCert Successful");
		} else {
			LogKit.info("WARN: acpsdk.encryptCert.path is empty");
		}
	}
	...
	private void initTrackKey() {
		if (!isEmpty(sdkConfig.getEncryptTrackKeyModulus()) && !isEmpty(sdkConfig.getEncryptTrackKeyExponent())) {
			encryptTrackKey = getPublicKey(sdkConfig.getEncryptTrackKeyModulus(),
					sdkConfig.getEncryptTrackKeyExponent());
			LogKit.info("LoadEncryptTrackKey Successful");
		} else {
			LogKit.info("WARN: acpsdk.encryptTrackKey.modulus or acpsdk.encryptTrackKey.exponent is empty");
		}
	}
	...
	private void initValidateCertFromDir() {
		...
		try {
			...
			LogKit.error("LoadVerifyCert Error: No BC Provider", e);
			return;
		} catch (CertificateException e) {
			LogKit.error("LoadVerifyCert Error", e);
			return;
		}
		...
		for (int i = 0; i < files.length; i++) {
			...
			try {
				...
				if (validateCert == null) {
					LogKit.error("Load verify cert error, " + file.getAbsolutePath() + " has error cert content.");
					continue;
				}
				...
				LogKit.error("LoadVerifyCert Error File Not Found", e);
			} finally {
				if (null != in) {
					try {
						in.close();
		...
		LogKit.info("LoadVerifyCert Finish");
	}
	...
	private static void loadSignCert(String certFilePath, String certPwd) {
		...
		try {
			...
			LogKit.info("LoadRsaCert Successful");
		} catch (IOException e) {
			LogKit.error("LoadRsaCert Error", e);
		}
	}
	...
	private static X509Certificate initCert(String path) {
		...
		try {
			...
			// 打印证书加载信息,供测试阶段调试
			LogKit.info("[" + path + "][CertId=" + encryptCertTemp.getSerialNumber().toString() + "]");
		} catch (CertificateException e) {
			LogKit.error("InitCert Error", e);
		} catch (FileNotFoundException e) {
			LogKit.error("InitCert Error File Not Found", e);
		} catch (NoSuchProviderException e) {
			LogKit.error("LoadVerifyCert Error No BC Provider", e);
		} finally {
			if (null != in) {
	...
	public PrivateKey getSignCertPrivateKey() {
		try {
			...
			PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias, sdkConfig.getSignCertPwd().toCharArray());
			return privateKey;
		} catch (KeyStoreException e) {
			LogKit.error("getSignCertPrivateKey Error", e);
			return null;
	...
	public static PrivateKey getSignCertPrivateKeyByStoreMap(String certPath, String certPwd) {
		...
		try {
			...
			LogKit.error("getSignCertPrivateKeyByStoreMap Error", e);
			return null;
		} catch (UnrecoverableKeyException e) {
			LogKit.error("getSignCertPrivateKeyByStoreMap Error", e);
			return null;
	...
	public PublicKey getEncryptCertPublicKey() {
		if (null == encryptCert) {
			...
			if (!isEmpty(path)) {
				...
				LogKit.error("acpsdk.encryptCert.path is empty");
				return null;
			}
		} else {
			return encryptCert.getPublicKey();
	...
	public PublicKey getValidatePublicKey(String certId) {
		...
		if (certMap.containsKey(certId)) {
			...
			if (certMap.containsKey(certId)) {
				...
				LogKit.error("缺少certId=[" + certId + "]对应的验签证书.");
				return null;
			}
		}
	}
	...
	public String getSignCertId() {
		try {
			...
			LogKit.error("getSignCertId Error", e);
			return null;
		}
	}
	...
	private static KeyStore getKeyInfo(String pfxkeyfile, String keypwd, String type) throws IOException {
		LogKit.info("加载签名证书==>" + pfxkeyfile);
		FileInputStream fis = null;
		try {
			KeyStore ks = KeyStore.getInstance(type, "BC");
			LogKit.info("Load RSA CertPath=[" + pfxkeyfile + "],Pwd=[" + keypwd + "],type=[" + type + "]");
			...
			LogKit.error("getKeyInfo Error", e);
			return null;
		} finally {
			if (null != fis)
				fis.close();
	...
	private static String getCertIdIdByStore(KeyStore keyStore) {
		...
		try {
			...
			LogKit.error("getCertIdIdByStore Error", e);
			return null;
		}
	}
	...
	public static X509Certificate genCertificateByStr(String x509CertString) {
		...
		try {
			...
			LogKit.error("gen certificate error", e);
		}
		return x509Cert;
	}
	...
	public X509Certificate getMiddleCert() {
		if (null == middleCert) {
			...
			if (!isEmpty(path)) {
				...
				LogKit.error(SDKConfig.SDK_MIDDLECERT_PATH + " not set in " + sdkConfig.getPropFileName());
				return null;
			}
		}
		return middleCert;
	...
	public X509Certificate getRootCert() {
		if (null == rootCert) {
			...
			if (!isEmpty(path)) {
				...
				LogKit.error(SDKConfig.SDK_ROOTCERT_PATH + " not set in " + sdkConfig.getPropFileName());
				return null;
			}
		}
		return rootCert;
	...
	private boolean verifyCertificateChain(X509Certificate cert) {
		...
		if (null == cert) {
			LogKit.error("cert must Not null");
			return false;
		}

		X509Certificate middleCert = getMiddleCert();
		if (null == middleCert) {
			LogKit.error("middleCert must Not null");
			return false;
		}

		X509Certificate rootCert = getRootCert();
		if (null == rootCert) {
			LogKit.error("rootCert or cert must Not null");
			return false;
		}
		...
		try {
			...
			LogKit.info("verify certificate chain succeed.");
			return true;
		} catch (java.security.cert.CertPathBuilderException e) {
			LogKit.error("verify certificate chain fail.", e);
		} catch (Exception e) {
			LogKit.error("verify certificate chain exception: ", e);
		}
		return false;
	}
	...
	public boolean verifyCertificate(X509Certificate cert) {
		...
		try {
			...
			LogKit.error("verifyCertificate fail", e);
			return false;
		}
		...
		if (sdkConfig.isIfValidateCNName()) {
			...
			if (!UNIONPAY_CNNAME.equals(CertUtil.getIdentitiesFromCertficate(cert))) {
				LogKit.error("cer owner is not CUP:" + CertUtil.getIdentitiesFromCertficate(cert));
				return false;
			}
		} else {
			// 验证公钥是否属于银联
	...
	private static void printSysInfo() {
		LogKit.info("================= SYS INFO begin====================");
		LogKit.info("os_name:" + System.getProperty("os.name"));
		LogKit.info("os_arch:" + System.getProperty("os.arch"));
		LogKit.info("os_version:" + System.getProperty("os.version"));
		LogKit.info("java_vm_specification_version:" + System.getProperty("java.vm.specification.version"));
		LogKit.info("java_vm_specification_vendor:" + System.getProperty("java.vm.specification.vendor"));
		LogKit.info("java_vm_specification_name:" + System.getProperty("java.vm.specification.name"));
		LogKit.info("java_vm_version:" + System.getProperty("java.vm.version"));
		LogKit.info("java_vm_name:" + System.getProperty("java.vm.name"));
		LogKit.info("java.version:" + System.getProperty("java.version"));
		LogKit.info("java.vm.vendor=[" + System.getProperty("java.vm.vendor") + "]");
		LogKit.info("java.version=[" + System.getProperty("java.version") + "]");
		printProviders();
		LogKit.info("================= SYS INFO end=====================");
	}
	...
	private static void printProviders() {
		LogKit.info("Providers List:");
		Provider[] providers = Security.getProviders();
		for (int i = 0; i < providers.length; i++) {
			LogKit.info(i + 1 + "." + providers[i].getName());
		}
	}
